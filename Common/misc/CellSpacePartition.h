...
CellSpacePartition<entity>::CellSpacePartition(double width,    // width of 2D space
    ...
    m_dSpaceWidth(width), m_dSpaceHeight(height), m_iNumCellsX(cellsX), m_iNumCellsY(cellsY), m_Neighbors(MaxEntitys,
            entity())
{
...
void CellSpacePartition<entity>::CalculateNeighbors(Vector2D TargetPos, double QueryRadius)
{
    ...
    // area
    InvertedAABBox2D QueryBox(TargetPos - Vector2D(QueryRadius, QueryRadius), TargetPos + Vector2D(QueryRadius,
                              QueryRadius));
...
{
    int idx = (int)(m_iNumCellsX * pos.x / m_dSpaceWidth) + ((int)((m_iNumCellsY) * pos.y / m_dSpaceHeight) *
              m_iNumCellsX);
