...
public:
    FuzzySet_Triangle(std::string name, double mid, double lft, double rgt): FuzzySet(name), m_dMidPoint(mid),
        m_dLeftOffset(lft), m_dRightOffset(rgt) {}
...
inline double FuzzySet_Triangle::CalculateDOM(double val)
{
    ...
    if ((isEqual(m_dRightOffset, 0.0) && (isEqual(m_dMidPoint, val))) || (isEqual(m_dLeftOffset, 0.0) && (isEqual(
                m_dMidPoint, val))))
    {
        return 1.0;
    }
