...
public:
    ...
    int NumActiveNodes() const
    {
        ...
        for (unsigned int n = 0; n < m_Nodes.size(); ++n) if (m_Nodes[n].Index() != invalid_node_index)
            {
                ++count;
            }
        return count;
...
{
    assert((from < m_Nodes.size()) && (from >= 0) && m_Nodes[from].Index() != invalid_node_index &&
...
void SparseGraph<node_type, edge_type>::CullInvalidEdges()
{
    for (typename EdgeListVector::iterator curEdgeList = m_Edges.begin(); curEdgeList != m_Edges.end(); ++curEdgeList)
    {
        ...
        {
            if (m_Nodes[curEdge->To()].Index() == invalid_node_index || m_Nodes[curEdge->From()].Index() ==
                    invalid_node_index)
            {
