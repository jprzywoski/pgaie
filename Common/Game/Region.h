...
public:
    ...
    Region(double left, double top, double right, double bottom, int id = -1): m_dTop(top), m_dRight(right), m_dLeft(
            left), m_dBottom(bottom), m_iID(id)
    {
...
inline bool Region::Inside(Vector2D pos, region_modifier r = normal) const
{
    ...
    else
    {
        ...
        return (pos.x > (m_dLeft + marginX)) && (pos.x < (m_dRight - marginX)) && (pos.y > (m_dTop + marginY)) &&
               (pos.y < (m_dBottom - marginY));
    }
}
