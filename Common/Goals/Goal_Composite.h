...
int Goal_Composite<entity_type>::ProcessSubgoals()
{
    ...
    if (!m_SubGoals.empty())
    {
        ...
        if (StatusOfSubGoals == Goal_Composite<entity_type>::completed && m_SubGoals.size() > 1)
        {
            return Goal_Composite<entity_type>::active;
        }
    ...
    {
        return Goal_Composite<entity_type>::completed;
    }
}
