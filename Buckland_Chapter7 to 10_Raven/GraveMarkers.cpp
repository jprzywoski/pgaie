...
GraveMarkers::GraveMarkers(double lifetime): m_dLifeTime(lifetime)
{
    ...
    const Vector2D rip[NumripVerts] = { Vector2D(-4, -5), Vector2D(-4, 3), Vector2D(-3, 5), Vector2D(-1, 6), Vector2D(1,
                                        6), Vector2D(3, 5), Vector2D(4, 3), Vector2D(4, -5), Vector2D(-4, -5)
                                      };
...
void GraveMarkers::Render()
{
    ...
    {
        m_vecRIPVBTrans = WorldTransform(m_vecRIPVB, it->Position, facing, facing.Perp(), Vector2D(1, 1));
