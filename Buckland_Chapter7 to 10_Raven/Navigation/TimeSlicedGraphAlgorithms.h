...
enum
{
    target_found, target_not_found, search_incomplete
};
...
public:
    Graph_SearchAStar_TS(const graph_type &G, int source, int target): Graph_SearchTimeSliced<Edge>(
            Graph_SearchTimeSliced<Edge>::AStar), m_Graph(G), m_ShortestPathTree(G.NumNodes()), m_SearchFrontier(
                G.NumNodes()), m_GCosts(G.NumNodes(), 0.0), m_FCosts(G.NumNodes(), 0.0), m_iSource(source), m_iTarget(target)
    {
        // create the PQ
...
std::list<PathEdge> Graph_SearchAStar_TS<graph_type, heuristic>::GetPathAsPathEdges() const
{
    ...
    while ((nd != m_iSource) && (m_ShortestPathTree[nd] != 0))
    {
        path.push_front(PathEdge(m_Graph.GetNode(m_ShortestPathTree[nd]->From()).Pos(), m_Graph.GetNode(
                                     m_ShortestPathTree[nd]->To()).Pos(), m_ShortestPathTree[nd]->Flags(),
...
public:
    Graph_SearchDijkstras_TS(const graph_type &G, int source, int target): Graph_SearchTimeSliced<Edge>(
            Graph_SearchTimeSliced<Edge>::Dijkstra), m_Graph(G), m_ShortestPathTree(G.NumNodes()), m_SearchFrontier(
                G.NumNodes()), m_CostToThisNode(G.NumNodes(), 0.0), m_iSource(source), m_iTarget(target)
    {
        // create the PQ         ,
