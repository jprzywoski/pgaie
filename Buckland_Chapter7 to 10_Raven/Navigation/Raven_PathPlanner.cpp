...
// ---------------------------- ctor -------------------------------------------
// -----------------------------------------------------------------------------
Raven_PathPlanner::Raven_PathPlanner(Raven_Bot *owner): m_pOwner(owner), m_NavGraph(
        m_pOwner->GetWorld()->GetMap()->GetNavGraph()), m_pCurrentSearch(NULL) {}
...
double Raven_PathPlanner::GetCostToClosestItem(unsigned int GiverType) const
{
    ...
    for (it = triggers.begin(); it != triggers.end(); ++it)
    {
        ...
        {
            double cost = m_pOwner->GetWorld()->GetMap()->CalculateCostToTravelBetweenNodes(nd,
                          (*it)->GraphNodeIndex());
...
void Raven_PathPlanner::SmoothPathEdgesQuick(Path &path)
{
    ...
    while (e2 != path.end())
    {
        ...
        if ((e2->Behavior() == EdgeType::normal) && m_pOwner->canWalkBetween(e1->Source(), e2->Destination()))
        {
...
int Raven_PathPlanner::CycleOnce() const
{
    ...
    {
        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, SENDER_ID_IRRELEVANT, m_pOwner->ID(), Msg_NoPathAvailable,
                                NO_ADDITIONAL_INFO);
    }
    ...
    else if (result == target_found)
    {
        ...
        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, SENDER_ID_IRRELEVANT, m_pOwner->ID(), Msg_PathReady, pTrigger);
    }
    return result;
}
