...
// ----------------------------- ctor ------------------------------------------
// -----------------------------------------------------------------------------
Raven_Map::Raven_Map(): m_pNavGraph(NULL), m_pSpacePartition(NULL), m_iSizeY(0), m_iSizeX(0),
...
{
    assert(nd1 >= 0 && nd1 < m_pNavGraph->NumNodes() && nd2 >= 0 && nd2 < m_pNavGraph->NumNodes() &&
...
void Raven_Map::PartitionNavGraph()
{
    ...
    m_pSpacePartition = new CellSpacePartition<NavGraph::NodeType *>(m_iSizeX, m_iSizeY, script->GetInt("NumCellsX"),
            script->GetInt("NumCellsY"), m_pNavGraph->NumNodes());
