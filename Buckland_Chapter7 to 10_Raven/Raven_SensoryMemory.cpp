...
// ------------------------------- ctor ----------------------------------------
// -----------------------------------------------------------------------------
Raven_SensoryMemory::Raven_SensoryMemory(Raven_Bot *owner, double MemorySpan): m_pOwner(owner), m_dMemorySpan(
        MemorySpan) {}
    ...
    for (curBot = bots.begin(); curBot != bots.end(); ++curBot)
    {
        ...
        if (m_pOwner != *curBot)
        {
            ...
            if (m_pOwner->GetWorld()->isLOSOkay(m_pOwner->Pos(), (*curBot)->Pos()))
            {
                ...
                if (isSecondInFOVOfFirst(m_pOwner->Pos(), m_pOwner->Facing(), (*curBot)->Pos(),
