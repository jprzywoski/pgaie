...
// ---------------------------- ctor -------------------------------------------
// -----------------------------------------------------------------------------
Goal_NegotiateDoor::Goal_NegotiateDoor(Raven_Bot *pBot, PathEdge edge, bool LastEdge): Goal_Composite<Raven_Bot>(pBot,
            goal_negotiate_door), m_PathEdge(edge), m_bLastEdgeInPath(LastEdge) {}
...
void Goal_NegotiateDoor::Activate()
{
    ...
    Vector2D posSw = m_pOwner->GetWorld()->GetPosOfClosestSwitch(m_pOwner->Pos(), m_PathEdge.DoorID());
