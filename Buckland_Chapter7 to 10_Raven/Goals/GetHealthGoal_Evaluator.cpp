...
double GetHealthGoal_Evaluator::CalculateDesirability(Raven_Bot *pBot)
{
    ...
    else
    {
        ...
        double Desirability = Tweaker * (1 - Raven_Feature::Health(pBot)) / (Raven_Feature::DistanceToItem(pBot,
                              type_health));
...
void GetHealthGoal_Evaluator::RenderInfo(Vector2D Position, Raven_Bot *pBot)
{
    ...
    std::string s = ttos(1 - Raven_Feature::Health(pBot)) + ", " + ttos(Raven_Feature::DistanceToItem(pBot,
                    type_health));
