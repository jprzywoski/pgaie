...
Trigger_SoundNotify::Trigger_SoundNotify(Raven_Bot *source, double range): Trigger_LimitedLifetime<Raven_Bot>(
        FrameRate / script->GetInt("Bot_TriggerUpdateFreq")), m_pSoundSource(source)
{
...
void Trigger_SoundNotify::Try(Raven_Bot *pBot)
{
    ...
    if (isTouchingTrigger(pBot->Pos(), pBot->BRadius()))
        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, SENDER_ID_IRRELEVANT, pBot->ID(), Msg_GunshotSound,
                                m_pSoundSource);
}
