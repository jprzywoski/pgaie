...
public:
    Trigger_OnButtonSendMsg(std::ifstream &datafile): Trigger<entity_type>(GetValueFromStream<int>(datafile))
    {
        Read(datafile);
    }
...
void Trigger_OnButtonSendMsg<entity_type>::Try(entity_type *pEnt)
{
    ...
    {
        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, this->ID(), m_iReceiver, m_iMessageToSend, NO_ADDITIONAL_INFO);
    }
}
