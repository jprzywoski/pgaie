...
Trigger_WeaponGiver::Trigger_WeaponGiver(std::ifstream &datafile): Trigger_Respawning<Raven_Bot>(
        GetValueFromStream<int>(datafile))
{
    Read(datafile);

    // create the vertex buffer for the rocket shape
    const int NumRocketVerts = 8;
    const Vector2D rip[NumRocketVerts] = { Vector2D(0, 3), Vector2D(1, 2), Vector2D(1, 0), Vector2D(2, -2), Vector2D(-2,
                                           -2), Vector2D(-1, 0), Vector2D(-1, 2), Vector2D(0, 3)
                                         };
...
void Trigger_WeaponGiver::Render()
{
    if (isActive())
    {
        ...
        case type_rocket_launcher:
        {
            ...
            m_vecRLVBTrans = WorldTransform(m_vecRLVB, Pos(), facing, facing.Perp(), Vector2D(2.5, 2.5));
