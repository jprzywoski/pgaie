...
// ----------------------------- ctor ------------------------------------------
// -----------------------------------------------------------------------------
Raven_Game::Raven_Game(): m_pSelectedBot(NULL), m_bPaused(false), m_bRemoveABot(false), m_pMap(NULL), m_pPathManager(
        NULL), m_pGraveMarkers(NULL)
{
...
void Raven_Game::Clear()
{
    ...
    {
#ifdef LOG_CREATIONAL_STUFF
        debug_con << "deleting entity id: " << (*it)->ID() << " of type " << GetNameOfType((*it)->EntityType()) <<
                  "(" << (*it)->EntityType() << ")" << "";
#endif
...
void Raven_Game::NotifyAllBotsOfRemoval(Raven_Bot *pRemovedBot) const
{
    ...
    for (curBot; curBot != m_Bots.end(); ++curBot)
    {
        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, SENDER_ID_IRRELEVANT, (*curBot)->ID(), Msg_UserHasRemovedBot,
                                pRemovedBot);
    }
}
...
bool Raven_Game::isSecondVisibleToFirst(const Raven_Bot *pFirst, const Raven_Bot *pSecond) const
{
    ...
    {
        // first of all test to see if this bot is within the FOV
        if (isSecondInFOVOfFirst(pFirst->Pos(), pFirst->Facing(), pSecond->Pos(), pFirst->FieldOfView()))
        {
