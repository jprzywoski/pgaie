...
// -------------------------- ctor ---------------------------------------------
// -----------------------------------------------------------------------------
Rocket::Rocket(Raven_Bot *shooter, Vector2D target): Raven_Projectile(target, shooter->GetWorld(), shooter->ID(),
            shooter->Pos(), shooter->Facing(), script->GetInt("Rocket_Damage"), script->GetDouble("Rocket_Scale"),
            script->GetDouble("Rocket_MaxSpeed"), script->GetDouble("Rocket_Mass"), script->GetDouble("Rocket_MaxForce")),
    m_dCurrentBlastRadius(0.0), m_dBlastRadius(script->GetDouble("Rocket_BlastRadius"))
{
...
void Rocket::TestForImpact()
{
    ...
    if (hit)
    {
        ...
        // shot came from
        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, m_iShooterID, hit->ID(), Msg_TakeThatMF,
    ...
    if (FindClosestPointOfIntersectionWithWalls(m_vPosition - m_vVelocity, m_vPosition, dist, m_vImpactPoint,
...
void Rocket::InflictDamageOnBotsWithinBlastRadius()
{
    ...
    for (curBot; curBot != m_pWorld->GetAllBots().end(); ++curBot)
    {
        if (Vec2DDistance(Pos(), (*curBot)->Pos()) < m_dBlastRadius + (*curBot)->BRadius())
        {
            // send a message to the bot to let it know it's been hit, and who the
            // shot came from
            Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, m_iShooterID, (*curBot)->ID(), Msg_TakeThatMF,
                                    (void *)&m_iDamageInflicted);
        }
    }
}
