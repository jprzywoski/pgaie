...
// -------------------------- ctor ---------------------------------------------
// -----------------------------------------------------------------------------
Slug::Slug(Raven_Bot *shooter, Vector2D target): Raven_Projectile(target, shooter->GetWorld(), shooter->ID(),
            shooter->Pos(), shooter->Facing(), script->GetInt("Slug_Damage"), script->GetDouble("Slug_Scale"),
            script->GetDouble("Slug_MaxSpeed"), script->GetDouble("Slug_Mass"), script->GetDouble("Slug_MaxForce")),
...
void Slug::TestForImpact()
{
    ...
    for (it = hits.begin(); it != hits.end(); ++it)
    {
        ...
        // shot came from
        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, m_iShooterID, (*it)->ID(), Msg_TakeThatMF,
