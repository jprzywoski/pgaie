...
// --------------------------- ctor --------------------------------------------
// -----------------------------------------------------------------------------
ShotGun::ShotGun(Raven_Bot *owner): Raven_Weapon(type_shotgun, script->GetInt("ShotGun_DefaultRounds"), script->GetInt(
                "ShotGun_MaxRoundsCarried"), script->GetDouble("ShotGun_FiringFreq"), script->GetDouble("ShotGun_IdealRange"),
            script->GetDouble("Pellet_MaxSpeed"), owner), m_iNumBallsInShell(script->GetInt("ShotGun_NumBallsInShell")),
    m_dSpread(script->GetDouble("ShotGun_Spread"))
{
    // setup the vertex buffer
    const int NumWeaponVerts = 8;
    const Vector2D weapon[NumWeaponVerts] = { Vector2D(0, 0), Vector2D(0, -2), Vector2D(10, -2), Vector2D(10, 0),
                                              Vector2D(0, 0), Vector2D(0, 2), Vector2D(10, 2), Vector2D(10, 0)
                                            };
...
void ShotGun::InitializeFuzzyModule()
{
    ...
    const FzSet &Target_Close = DistanceToTarget.AddLeftShoulderSet("Target_Close", 0, 25, 150);
    const FzSet &Target_Medium = DistanceToTarget.AddTriangularSet("Target_Medium", 25, 150, 300);
    const FzSet &Target_Far = DistanceToTarget.AddRightShoulderSet("Target_Far", 150, 300, 1000);
    FuzzyVariable &Desirability = m_FuzzyModule.CreateFLV("Desirability");
    const FzSet &VeryDesirable = Desirability.AddRightShoulderSet("VeryDesirable", 50, 75, 100);
    const FzSet &Desirable = Desirability.AddTriangularSet("Desirable", 25, 50, 75);
    const FzSet &Undesirable = Desirability.AddLeftShoulderSet("Undesirable", 0, 25, 50);
    FuzzyVariable &AmmoStatus = m_FuzzyModule.CreateFLV("AmmoStatus");
    const FzSet &Ammo_Loads = AmmoStatus.AddRightShoulderSet("Ammo_Loads", 30, 60, 100);
    const FzSet &Ammo_Okay = AmmoStatus.AddTriangularSet("Ammo_Okay", 0, 30, 60);
    const FzSet &Ammo_Low = AmmoStatus.AddTriangularSet("Ammo_Low", 0, 0, 30);
...
void ShotGun::Render()
{
    m_vecWeaponVBTrans = WorldTransform(m_vecWeaponVB, m_pOwner->Pos(), m_pOwner->Facing(), m_pOwner->Facing().Perp(),
                                        m_pOwner->Scale());
