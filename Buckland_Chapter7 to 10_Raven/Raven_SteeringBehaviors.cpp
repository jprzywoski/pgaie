...
//
// ------------------------------------------------------------------------
Raven_Steering::Raven_Steering(Raven_Game *world, Raven_Bot *agent): m_pWorld(world), m_pRaven_Bot(agent), m_iFlags(0),
    m_dWeightSeparation(script->GetDouble("SeparationWeight")), m_dWeightWander(script->GetDouble("WanderWeight")),
    m_dWeightWallAvoidance(script->GetDouble("WallAvoidanceWeight")), m_dViewDistance(script->GetDouble(
                "ViewDistance")), m_dWallDetectionFeelerLength(script->GetDouble("WallDetectionFeelerLength")), m_Feelers(3),
    m_Deceleration(normal), m_pTargetAgent1(NULL), m_pTargetAgent2(NULL), m_dWanderDistance(WanderDist),
    m_dWanderJitter(WanderJitterPerSec), m_dWanderRadius(WanderRad), m_dWeightSeek(script->GetDouble("SeekWeight")),
    m_dWeightArrive(script->GetDouble("ArriveWeight")), m_bCellSpaceOn(false), m_SummingMethod(prioritized)
{
...
{
    Vector2D DesiredVelocity = Vec2DNormalize(target - m_pRaven_Bot->Pos()) * m_pRaven_Bot->MaxSpeed();
...
Vector2D Raven_Steering::Wander()
{
    ...
    Vector2D Target = PointToWorldSpace(target, m_pRaven_Bot->Heading(), m_pRaven_Bot->Side(), m_pRaven_Bot->Pos());
...
Vector2D Raven_Steering::WallAvoidance(const vector<Wall2D *> &walls)
{
    ...
    for (unsigned int flr = 0; flr < m_Feelers.size(); ++flr)
    {
        ...
        {
            if (LineIntersection2D(m_pRaven_Bot->Pos(), m_Feelers[flr], walls[w]->From(), walls[w]->To(), DistToThisIP,
                                   point))
            {
...
void Raven_Steering::CreateFeelers()
{
    ...
    m_Feelers[0] = m_pRaven_Bot->Pos() + m_dWallDetectionFeelerLength * m_pRaven_Bot->Heading() * m_pRaven_Bot->Speed();
