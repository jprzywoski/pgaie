...
// points accordingly
// -----------------------------------------------------------------------------
void EvaluateTheGuesses(std::string user_guess, std::string comp_guess, int &user_score, int &comp_score)
{
...
int cpp_EvaluateTheGuesses(lua_State *pL)
{
    ...
    if (!lua_isstring(pL, 1) || !lua_isstring(pL, 2) || !lua_isnumber(pL, 3) || !lua_isnumber(pL, 4))
    {
