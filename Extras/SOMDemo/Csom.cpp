...
void Csom::Create(int cxClient, int cyClient, int CellsUp, int CellsAcross, int NumIterations)
{
    ...
    if (--m_iNumIterations > 0)
    {
        ...
        for (int n = 0; n < m_SOM.size(); ++n)
        {
            ...
            if (DistToNodeSq < (m_dNeighbourhoodRadius * m_dNeighbourhoodRadius))
            {
                ...
                m_SOM[n].AdjustWeights(data[ThisVector], m_dLearningRate, m_dInfluence);
            }
        } // next node
