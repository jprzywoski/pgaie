...
// ------------------------------------------------------------------------
void CController::Init(int cyClient, int cxClient, int NumTWinks, int minRad, int maxRad)
{
    m_cxClient = cxClient;
    m_cyClient = cyClient;
    ...
    for (int t = 0; t < NumTWinks; ++t)
    {
        ...
        while (bOverlapped)
        {
            STiddlywink tw(RandInt(radius, m_cxClient - radius), RandInt(radius, m_cyClient - radius - 30), radius);
    ...
    // inititialize the GA
    m_pGA = new Cga(CROSSOVER_RATE, MUTATION_RATE, POP_SIZE, CHROMO_LENGTH, GENE_LENGTH);
...
bool CController::Overlapped(const STiddlywink &tw)
{
    ...
    {
        if (TwoCirclesOverlapped(tw.x, tw.y, tw.r, m_vecTW[t].x, m_vecTW[t].y, m_vecTW[t].r))
        {
            return true;
        }
    }
...
void CController::Epoch()
{
    ...
    for (int p = 0; p < m_vecPop.size(); ++p)
    {
        ...
        for (int i = 0; i < m_vecTW.size(); ++i)
        {
            ...
            // enclosing a circle
            if (TwoCirclesOverlapped(m_vecTW[i].x, m_vecTW[i].y, m_vecTW[i].r, x, y, r))
            {
                radius = 0;
                break;
            }
