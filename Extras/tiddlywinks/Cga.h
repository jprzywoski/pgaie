...
private:
    ...
    void Crossover(const vector<int> &mum, const vector<int> &dad, vector<int> &baby1, vector<int> &baby2);

    SGenome &RouletteWheelSelection();

    void GrabNBest(int NBest, const int NumCopies, vector<SGenome> &vecNewPop);
...
public:
    Cga(double cross_rat, double mut_rat, int pop_size, int num_bits, int gene_len): m_dCrossoverRate(cross_rat),
        m_dMutationRate(mut_rat), m_iPopSize(pop_size), m_iChromoLength(num_bits), m_dTotalFitnessScore(0.0),
        m_iGeneration(0), m_iGeneLength(gene_len)
    {
    ...
    // accessor methods
    vector<SGenome> GrabGenomes()
    {
        return m_vecGenomes;
    }
    void PutGenomes(vector<SGenome> gen)
    {
        m_vecGenomes = gen;
    }
};
