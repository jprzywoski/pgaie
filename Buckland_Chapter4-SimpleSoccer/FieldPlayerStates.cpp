...
bool GlobalPlayerState::OnMessage(FieldPlayer *player, const Telegram &telegram)
{
    ...
    case Msg_PassToMe:
    {
        ...
        Vector2D temp = receiver->Pos();

        Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY, player->ID(), receiver->ID(), Msg_ReceiveBall, &temp);
...
void SupportAttacker::Execute(FieldPlayer *player)
{
    ...
    if (player->Team()->CanShoot(player->Pos(), Prm.MaxShootingForce, temp))
    {
...
void KickBall::Execute(FieldPlayer *player)
{
    ...
    // to make the shot
    if (player->Team()->CanShoot(player->Ball()->Pos(), power, BallTarget) || (RandFloat() <
            Prm.ChancePlayerAttemptsPotShot))
    {
#ifdef PLAYER_STATE_INFO_ON
