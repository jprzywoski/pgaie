...
void SoccerBall::TestCollisionWithWalls(const std::vector<Wall2D> &walls)
{
    ...
    for (unsigned int w = 0; w < walls.size(); ++w)
    {
        ...
        {
            double DistToWall = DistanceToRayPlaneIntersection(ThisCollisionPoint, walls[w].Normal(), walls[w].From(),
                                walls[w].Normal());
        ...
        {
            double DistToWall = DistanceToRayPlaneIntersection(ThisCollisionPoint, VelNormal, walls[w].From(),
                                walls[w].Normal());
        ...
        if (LineIntersection2D(walls[w].From(), walls[w].To(), ThisCollisionPoint - walls[w].Normal() * 20.0,
