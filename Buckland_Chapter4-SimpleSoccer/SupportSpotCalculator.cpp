...
// ------------------------------- ctor ----------------------------------------
// -----------------------------------------------------------------------------
SupportSpotCalculator::SupportSpotCalculator(int numX, int numY, SoccerTeam *team): m_pBestSupportingSpot(NULL),
    m_pTeam(team)
{
...
Vector2D SupportSpotCalculator::DetermineBestSupportingPosition()
{
    ...
    for (curSpot = m_Spots.begin(); curSpot != m_Spots.end(); ++curSpot)
    {
        ...
        // to this position?
        if (m_pTeam->isPassSafeFromAllOpponents(m_pTeam->ControllingPlayer()->Pos(), curSpot->m_vPos, NULL,
                                                Prm.MaxPassingForce))
        {
        ...
        if (m_pTeam->CanShoot(curSpot->m_vPos, Prm.MaxShootingForce, temp))
        {
        ...
        if (m_pTeam->SupportingPlayer())
        {
            ...
            if (temp < OptimalDistance)
            {
                ...
                curSpot->m_dScore += Prm.Spot_DistFromControllingPlayerScore * (OptimalDistance - temp) /
                                     OptimalDistance;
            }
        }
